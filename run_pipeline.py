...
def signed(pdf_files, out_json, no_text_fingerprint=False, strict=False):
    files = [Path(value) for value in pdf_files]
    ...
    if not out_json:
        out_json = str(files[0].parent / "signed_package.json")
    command = [SCRIPTS / "inspect_signed_bid_package.py"] + [str(path) for path in files]
    command += ["--out", out_json]
    if no_text_fingerprint:
        command.append("--no-text-fingerprint")
    if strict:
        command.append("--strict")

    ok = run(command, "签章终稿 · PDF签名结构、覆盖范围与重复内容")
    if not ok:
        print("✗ 签章结构检查发现严格模式错误，详见上方输出。")
        sys.exit(1)
    print("\n✓ signed 完成！")
    print("  JSON: %s" % out_json)
    print("  注意:本结果不替代政采云CA验签、上传、加密或解密确认。")


def main():
    if len(sys.argv) < 3 or sys.argv[1] not in ("prep", "verify", "signed"):
        print(__doc__)
        sys.exit(1)
    ...
    if cmd == "prep":
        ...
        while i < len(rest):
            value = rest[i]
            if value == "--out" and i + 1 < len(rest):
                out = rest[i + 1]
                i += 2
                continue
            if value == "--no-text-fingerprint":
                no_text_fingerprint = True
                i += 1
                continue
            if value == "--strict":
                strict = True
            ...
            i += 1
        if not cleaned:
            print("用法: python run_pipeline.py signed <PDF...> [--out 输出.json] [--strict]")
            sys.exit(1)
        signed(cleaned, out, no_text_fingerprint, strict)
