...
- 三模式工作流：规则审标、已编制投标文件复核、签章PDF终稿复核。
- `references/bid-package/`：报价反向归组、证明材料证据链、签章终稿十检。
- `inspect_signed_bid_package.py`：输出PDF页数、签名域、`/SubFilter`、`/ByteRange`覆盖、签后追加字节、精确重复和标准化文本重复。
- `run_pipeline.py signed` 一键生成 `signed_package.json`。
- Codex `agents/openai.yaml` UI元数据与自动触发配置。

### Changed

- 同品牌型号一致性从“跨文件正向核对”升级为“正向逐项 + 品牌型号反向归组”双关卡。
- 质保、节能环保、3C、校准、本国产品和中小企业声明统一按出具主体、对象、数量、期限、生效条件和核验方式审查。
- 明确区分原厂直接承诺、投标人承诺中标后购买、商品页和已购买/已激活凭证。
- 明确本地PDF签名结构不替代政采云CA验签、上传、加密和解密确认。
