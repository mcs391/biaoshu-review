...
| 模式 | 输入 | 额外检查 | 交付 |
|---|---|---|---|
| 规则审标 | 招标文件 | 废标、评分、证明、▲/★、时间、合同 | Excel + Markdown |
| 已编制文件复核 | 招标文件 + 资格/商务技术/报价 | 跨文件勾稽、数量金额、品牌型号反向归组、声明函 | Excel + Markdown + 一致性结果 |
| 签章终稿复核 | 招标文件 + 最终签章PDF | 签名域/ByteRange、签后追加、证据链、质保、生效条件、声明真实性、平台边界 | Excel + Markdown + signed_package.json |

签章检查只报告本地PDF结构，不替代政采云CA验签、上传、加密和解密回执。
...
**当前版本:Community Edition v0.2.0(社区开源版)**——本仓库内容,**MIT 许可,免费使用 / 修改 / 商用**。
- 10 个核心程序(取数 / 撒网 / 补词×2 / 查漏 / 完整性 / 跨文件 / 同型号异价 / 签章PDF / Excel)+ 3 个辅助(环境自检 / 候选词入库 / 脱敏贡献)+ 一键编排 `run_pipeline.py`
- **投标终稿审核(v0.2.0 新增)**:增加已编制投标文件核对、同品牌同型号反向归组、证明链审查和签章 PDF 本地结构检查,并明确平台 CA 验签边界。
...
# 4b. 如果是签章终稿,再检查PDF签名结构和重复内容
python run_pipeline.py signed <资格.pdf> <商务技术.pdf> <报价.pdf> --out workspace/<项目>.signed_package.json
...
│   ├── inspect_signed_bid_package.py  # 签章PDF结构、覆盖和重复内容
...
- [x] 规则审标 + 已编制文件复核 + 签章终稿复核三模式
- [x] 10 大核心程序 + 3 辅助 + 一键编排(`run_pipeline.py`)+ 通用机制清单
