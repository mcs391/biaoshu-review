...
当投标文件已经导出并签章为 PDF，可对整包文件做本地结构复核：

```bash
python run_pipeline.py signed <资格审查.pdf> <商务技术.pdf> <报价.pdf> --out signed_package.json
```

该步骤检查 PDF 可读性、签名域、ByteRange 覆盖范围、签章后追加字节，以及精确重复和标准化文本重复。它不能替代政采云等采购平台上的 CA 验签、上传、加密和解密确认。
...
你的招标文件
 ...
 xxx.xlsx 交付
    │
    ▼
 run_pipeline.py signed   ← 签章终稿本地结构与重复内容复核
```
